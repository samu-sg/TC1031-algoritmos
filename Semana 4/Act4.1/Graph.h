...
void Graph<T>::BFS(int v) {
  ...
  while (!queue.isEmpty()) {
    ...
    if (representation) { // Lista de adyacencia, Complejidad O(V+E)
    ...
    else { // Matriz de adyacencia, Complejidad O(V^2)
      // Recorremos los vertices vecinos de v
      for (int u = 1; u <= numNodes; u++) {
        int c = adjMatrix[v][u];
        if (c != 0) {
...
void Graph<T>::DFS(int v) {
  ...
  // Marcar al vertice actual v como visitado y meterlo en el stack
  visited.insert(v);
  stack.push(v);
