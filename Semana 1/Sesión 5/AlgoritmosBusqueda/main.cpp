...
int binarySearch(vector<int> vectorSorted, int key, int &compara){
  ...
  while (low <= high){
    mid = low + (high-low)/2;
    compara++;
    if (key == vectorSorted[mid]) return mid;
    else if (key < vectorSorted[mid]) high = mid-1;
    else low = mid+1;
  } 
  return -1;
}

int binarySearchRecursive(vector<int> vectorSorted, int low, int high, int key, int &compara){
  int m;
  if (low > high) return -1;
  m = low + (high -low)/2;
  compara++;
  if (key == vectorSorted[m]) return m;
